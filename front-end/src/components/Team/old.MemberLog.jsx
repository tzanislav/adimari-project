...
function MemberLog({ memberId, incrementMs = 3600000 }) {
    ...
    bars.forEach((bar, index) => {
       ...
       // dayGroup.reverse(); // Reverse the order of bars for the current day
    });
    if (dayGroup.length > 0) groupedBars.push({ date: currentDay, bars: dayGroup });
                        ...
                        {group.bars.map((bar, idx) => {
                            ...
                            return (
                                <React.Fragment key={bar.index}>
                                    <VerticalBar fill={bar.total / maxTotal} text={showHour&&prevHour ? "|" + prevHour : ""} details = {time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} />
                                </React.Fragment>
                            );
                        })}
                    </div>
