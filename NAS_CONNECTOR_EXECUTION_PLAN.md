...
- **Phase 2B:** complete. The service and backend maintain a secure persistent, outbound WSS **presence** channel with strict `hello`/ack/ping/pong protocol, credential-rotation session invalidation, redacted Control Center status, and no file or job commands.
- **Next:** implement a durable job queue and safe job delivery. NAS indexing, transfers, thumbnails, browsing, sharing, and browser uploads remain later phases and are not yet exposed to users.
...
- The target server must be an `https://` backend origin with a valid certificate. Phase 2A uses authenticated HTTPS enrollment/heartbeats; Phase 2B derives a secure outbound WSS presence endpoint from that origin. The Control Center never disables TLS validation. Development-only local HTTPS must use a trusted development certificate.
...
| Share behaviour | Creating a NAS share returns the usual branded File Server link immediately and queues cache preparation. The share page displays “preparing” until the matching NAS version is available. An active share whose delivery copy has expired requeues preparation when visited. |
...
| Image thumbnails | Store small generated derivatives in `nas-thumbnails/` while their source image exists. Generate lazily on the Windows connector, bounded to a configured maximum dimension (initially 320 px), and invalidate/regenerate whenever the source version changes. |
...
6. Implement a durable local job queue/state store, then extend the channel with safe, idempotent job delivery so an interrupted connector can resume or safely report a job after restart.
...
2. Use filesystem change notifications for prompt updates, but add scheduled reconciliation scans and an overflow/error-triggered rescan; notifications alone are not a reliable catalogue source.
...
6. Record connector scan health: last successful full scan, last event processed, count of inaccessible items, and index lag.
...
1. Add `prepareOpen`, `prepareDownload`, and `prepareShare` requests that authorize access and resolve a file entry/version. An inline-compatible open or download returns a short-lived object-storage URL when current, or creates a cache job. A share returns the usual opaque File Server link immediately and creates a preparation job when necessary.
...
5. Add an authorized sibling-image endpoint that returns only the nearest previous/next previewable image in the current folder after permission filtering. Add thumbnail preparation/status endpoints that return only scoped thumbnail URLs.
6. Connector handles `generate_thumbnail` jobs by first checking an optional configured NAS thumbnail repository with a deterministic source-to-thumbnail mapping; if no matching thumbnail exists, it opens the exact stable source image, enforces input/decompression bounds, and creates a maximum-320-pixel thumbnail. It uploads the result to an opaque versioned key under `nas-thumbnails/`. Mark a thumbnail ready only after matching source version and object verification.
7. Implement the authenticated full-screen image lightbox: streamed fetch/Blob rendering for real loading progress when available, top download icon, bottom full-width previous/position/next bar, keyboard/focus handling, and sibling navigation. Reuse the same cache object for full-image presentation; the explicit download icon requests an attachment URL.
...
1. Test authorization boundaries, connector enrollment/revocation, path traversal, symlink/reparse-point escape, malformed jobs, cache access, signed URL expiry, upload collisions, network loss, NAS loss, and restart recovery.
2. Add integration tests using an isolated test root and test bucket/emulator; no destructive test may target production NAS data.
3. Add end-to-end browser tests for browse, search, cached/uncached download, image thumbnails, image lightbox, progress states, previous/next folder boundaries, upload, permissions, and offline behavior.
4. Configure monitoring/alerts for connector offline state, scan failures, failed jobs, queue growth, storage cost, upload/download throughput, and abnormal access patterns.
5. Write runbooks for connector installation/upgrades, credential rotation/revocation, retrying and cancelling jobs, restoring a failed staging job, cache cleanup, and incident response.
6. Pilot with a restricted root and a small user group. Measure cache hit rate, first-download time, indexing latency, failure rate, and NAS/upload bandwidth before expanding access.

Acceptance criteria: all automated tests pass; a security review has no unresolved high-severity finding; recovery runbooks are validated in a non-production environment; pilot success criteria are agreed before rollout.
